...
export async function POST(request: NextRequest) {
  try {
    ...
    const reserva = await prisma.reservaHaciendaNapolesTour.create({
      data: {
        codigoReserva,
        servicioId: 6,
    ...
    console.error('Error creating Hacienda Nápoles Tour reservation:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservas = await prisma.reservaHaciendaNapolesTour.findMany({
      where,
      include: {
        vehiculo: true,
        hotel: true,
    ...
    console.error('Error fetching Hacienda Nápoles Tour reservations:', error);
    return NextResponse.json(
      {
        success: false,
