...
export async function POST(request: NextRequest) {
  try {
    ...
    const reserva = await prisma.reservaAtvTour.create({
      data: {
        codigoReserva,
        servicioId: 9,
        ...
        cantidadMotos: cantidadMotos || 0,
        precioMotos: precioMotos || 0,
        vehiculoId,
        precioBase,
        precioVehiculo,
        precioTotal,
    ...
    console.error('Error creating ATV Tour reservation:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservas = await prisma.reservaAtvTour.findMany({
      where,
      include: {
        vehiculo: true,
        hotel: true,
    ...
    console.error('Error fetching ATV Tour reservations:', error);
    return NextResponse.json(
      {
        success: false,
