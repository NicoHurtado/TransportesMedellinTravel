...
export async function POST(request: NextRequest) {
  try {
    ...
    const reserva = await prisma.reservaCoffeeFarmTour.create({
      data: {
        codigoReserva,
        servicioId: 11,
    ...
    console.error('Error creating Coffee Farm Tour reservation:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservas = await prisma.reservaCoffeeFarmTour.findMany({
      where,
      include: {
        vehiculo: true,
        hotel: true,
    ...
    console.error('Error fetching Coffee Farm Tour reservations:', error);
    return NextResponse.json(
      {
        success: false,
