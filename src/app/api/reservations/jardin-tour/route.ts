...
export async function POST(request: NextRequest) {
  try {
    ...
    const reserva = await prisma.reservaJardinTour.create({
      data: {
        codigoReserva,
        servicioId: 10,
    ...
    console.error('Error creating Jardín Tour reservation:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservas = await prisma.reservaJardinTour.findMany({
      where,
      include: {
        vehiculo: true,
        hotel: true,
    ...
    console.error('Error fetching Jardín Tour reservations:', error);
    return NextResponse.json(
      {
        success: false,
