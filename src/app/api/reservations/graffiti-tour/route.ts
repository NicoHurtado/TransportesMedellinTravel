...
export async function POST(request: NextRequest) {
  try {
    ...
    const reserva = await prisma.reservaGraffitiTour.create({
      data: {
        codigoReserva,
        servicioId: 4,
    ...
    console.error('Error creating Graffiti Tour reservation:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservas = await prisma.reservaGraffitiTour.findMany({
      where,
      include: {
        vehiculo: true,
        hotel: true,
    ...
    console.error('Error fetching Graffiti Tour reservations:', error);
    return NextResponse.json(
      {
        success: false,
