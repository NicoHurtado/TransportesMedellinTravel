...
export async function POST(request: NextRequest) {
  try {
    ...
    const reserva = await prisma.reservaOccidenteTour.create({
      data: {
        codigoReserva,
        servicioId: 7,
    ...
    console.error('Error creating Occidente Tour reservation:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservas = await prisma.reservaOccidenteTour.findMany({
      where,
      include: {
        vehiculo: true,
        hotel: true,
    ...
    console.error('Error fetching Occidente Tour reservations:', error);
    return NextResponse.json(
      {
        success: false,
